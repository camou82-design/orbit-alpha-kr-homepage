import { ProductCandidate, SCREENING_CRITERIA, ScreeningReport, SourcingStatus, ScreeningStatus, UploadLockStatus, RawSourcedCandidate, ReviewReasonType, ReviewPriority, WatchlistReasonType } from './config';

const EXCHANGE_RATE = 190;

export class SourcingScreener {

  public mapRawToCandidate(raw: RawSourcedCandidate): ProductCandidate {
    const title = raw.translated_title || raw.source_title;
    
    // 위험 키워드 기반 리스크 추정
    const hasRisk = (keywords: string[]) => keywords.some(k => title.toLowerCase().includes(k) || raw.source_spec_text.toLowerCase().includes(k));
  ...
  public screen(product: ProductCandidate): ScreeningReport {
    const rejectionReasons: string[] = [];
    const approvalReasons: string[] = [];
    const detectedRiskKeywords: string[] = [];
    ...
    if (risks.legal) rejectionReasons.push('법적/인증/브랜드 리스크');
    if (risks.hard_return) rejectionReasons.push('파손/중량/회수불가 리스크');
    if (risks.safety) rejectionReasons.push('안전/위생 리스크');

    for (const forbidden of SCREENING_CRITERIA.FORBIDDEN_CATEGORIES) {
      if (product.category.includes(forbidden)) rejectionReasons.push(`금지 카테고리: ${forbidden}`);
    }
    for (const keyword of SCREENING_CRITERIA.RISK_KEYWORDS) {
    ...
    const calculateMargin = (salePrice: number, cost: number) => {
      const platformFee = salePrice * SCREENING_CRITERIA.PLATFORM_FEE_RATE;
      const paymentFee = salePrice * SCREENING_CRITERIA.PAYMENT_FEE_RATE;
      const returnBuffer = salePrice * SCREENING_CRITERIA.RETURN_BUFFER_RATE;
    ...
    const marketabilityReasons: string[] = [];
    if (product.has_search_demand && (product.title.includes('1/4') || product.title.includes('3/8'))) marketabilityReasons.push('규격형 검색어 존재');
    if (isConsumable) marketabilityReasons.push('반복구매/소모품 성격');
    if (product.is_hard_to_find_locally) marketabilityReasons.push('국내에서 구하기 불편한 부속품');
    if (product.has_sales_history) marketabilityReasons.push('판매 흔적 존재');
    ...
    let screeningStatus: ScreeningStatus = 'REJECTED';
    let uploadLockStatus: UploadLockStatus = 'NONE';
    let reviewReasonType: ReviewReasonType = 'NONE';
    let reviewPriority: ReviewPriority = 'NONE';
    let watchlistReasonType: WatchlistReasonType = 'NONE';

    // v3.6 상세 증거 체크
    if (!product.key_specifications || product.key_specifications === 'Size: Standard') missingEvidenceFields.push('source_spec_text 부족');
    if (!product.package_components || product.package_components === '') missingEvidenceFields.push('component_quantity 부족');
    if (!product.source_option_name || product.source_option_name === 'Type A') missingEvidenceFields.push('source_option_name 부족');
    if (supplierScore < 90) missingEvidenceFields.push('supplier_score_reason 부족');
    if (marginAnalysis.marginRate < 0.3) missingEvidenceFields.push('final_margin_rate 불확실');
    ...
    if (rejectionReasons.length > 0) {
      ...
      if (isTripodAdapter) {
        ...
        if (!tripodEvidenceOk) missingEvidenceFields.push('나사 규격 증거 부족');
      }
      ...
      if (isHighQuality && isMarketable) {
        ...
        if (!tripodEvidenceOk && isTripodAdapter) reviewReasonType = 'SPEC_MISSING';
        else if (missingEvidenceFields.includes('source_option_name 부족')) reviewReasonType = 'OPTION_MISSING';
        else if (missingEvidenceFields.includes('component_quantity 부족')) reviewReasonType = 'COMPONENT_MISSING';
        else if (supplierScore < 90) reviewReasonType = 'SUPPLIER_EVIDENCE_WEAK';
        else if (marginAnalysis.marginRate < 0.3) reviewReasonType = 'MARGIN_UNCERTAIN';
        else reviewReasonType = 'DETAIL_PAGE_NEEDS_WORK';
    ...
    return this.finalizeReport(
      product, screeningStatus, uploadLockStatus, rejectionReasons, approvalReasons, detectedRiskKeywords, 
      marginAnalysis.marginRate, marginAnalysis, reviewReasonType, reviewPriority, watchlistReasonType, 
      missingEvidenceFields, visibilityScore, csBurdenScore, supplierScore, supplierAnalysis.reason
    );
  }

  private analyzeSupplierScore(product: ProductCandidate): { score: number, reason: string } {
    ...
    if (reviewCount > 100) { score += 5; reasons.push('리뷰 100건 이상'); }
    if (rating > 4.8) { score += 5; reasons.push('평점 4.8 초과'); }
    if (product.shipping_reliability_hint === 'FAST') { score += 5; reasons.push('배송 신뢰도 높음'); }
    if (reviewCount < 10 && rating < 4.5) { score -= 20; reasons.push('리뷰 및 평점 근거 부족'); }

    return { score: Math.min(100, score), reason: reasons.join(', ') || '특이사항 없음' };
  }

  private calculateVisibilityScore(product: ProductCandidate): number {
    let score = 50;
    if (product.title.includes('1/4') || product.title.includes('3/8')) score += 20;
    if (product.collected_keyword && product.title.includes(product.collected_keyword)) score += 20;
    return Math.min(100, score);
  }

  private calculateCSBurdenScore(product: ProductCandidate): number {
    let score = 30;
    if (product.is_unclear_compatibility) score += 30;
    if (product.category.includes('카메라')) score += 10;
    return Math.min(100, score);
  }

  private finalizeReport(
    product: ProductCandidate, screeningStatus: ScreeningStatus, uploadLockStatus: UploadLockStatus, 
    rejectionReasons: string[], approvalReasons: string[], keywords: string[], marginRate: number, 
    marginAnalysis: any, reviewReasonType: ReviewReasonType, reviewPriority: ReviewPriority, 
    watchlistReasonType: WatchlistReasonType, missingEvidenceFields: string[],
    visibilityScore: number, csBurdenScore: number, supplierScore: number, supplierScoreReason: string
  ): ScreeningReport {
