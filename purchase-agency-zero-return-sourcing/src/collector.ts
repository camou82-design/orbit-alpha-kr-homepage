...
import { RawSourcedCandidate, KeywordMetadata } from './config';

export class ProductCollector {
  private usedUrls: Set<string> = new Set();
  private usedTitles: Set<string> = new Set();

  async collect(keywordMeta: KeywordMetadata[]): Promise<RawSourcedCandidate[]> {
    const results: RawSourcedCandidate[] = [];
  ...
  private createCandidate(meta: KeywordMetadata, index: number, status: 'SUCCESS' | 'FAILED' | 'DUPLICATE'): RawSourcedCandidate {
    const keyword = meta.keyword;
