...
async function main() {
    ...
    const records = (0, sync_1.parse)(rawData, {
        columns: true,
        skip_empty_lines: true,
    });
    const screener = new screener_1.SourcingScreener();
    ...
    const keywordStats = keywords.map(kw => {
        ...
        kReports.forEach(r => {
            ...
            if (r.review_reason_type && r.review_reason_type !== 'NONE')
                reasons.set(r.review_reason_type, (reasons.get(r.review_reason_type) || 0) + 1);
        });
        const topReasons = Array.from(reasons.entries()).sort((a, b) => b[1] - a[1]).slice(0, 2).map(e => e[0]);
        let status = '⏳ 관찰중';
        if (app > 0 || high > 0)
            status = '✅ 확장권장';
        else if (total > 30 && rej / total > 0.9)
            status = '❌ 퇴출권장';
    ...
    await generateReports(reports, approved, nearApproved, reviewOnly, watchlist, rejected, keywordStats, reportPath, sourcingReportPath, successPatternPath);
    // v3.7 Enrichment Runner 가동
    const enricher = new enricher_1.EvidenceEnricher();
...
async function generateReports(reports, approved, nearApproved, reviewOnly, watchlist, rejected, keywordStats, screeningPath, sourcingPath, successPatternPath) {
...
`;
    ...
    const enrichmentQueue = nearApproved.map(item => ({
    ...
    const enrichmentRows = enrichmentQueue.map(q => `"${q.product_id}","${q.product_name.replace(/"/g, '""')}","${q.source_url}","${q.review_reason_type}","${q.missing_evidence_fields}","${q.required_action}","${q.current_screening_status}","${q.target_status_after_enrichment}",${q.human_check_required},"${q.memo}"`).join('\n');
    ...
    if (approved.length > 0) {
        ...
        expandedKeywords.push({
            parent_keyword: best.collected_keyword || 'Unknown',
    ...
    const keywordRows = expandedKeywords.map(k => `"${k.parent_keyword}","${k.expanded_keyword}","${k.reason}","${k.expected_category}","${k.risk_expectation}","${k.cs_risk_expectation}",${k.active},${k.test_limit}`).join('\n');
...
function generateEnrichmentReport(result) {
    const date = new Date().toISOString().split('T')[0];
    const enriched = fs.existsSync('data/enriched_candidates.csv') ? (0, sync_1.parse)(fs.readFileSync('data/enriched_candidates.csv', 'utf-8'), { columns: true }) : [];
    const failed = fs.existsSync('data/enrichment_failed_candidates.csv') ? (0, sync_1.parse)(fs.readFileSync('data/enrichment_failed_candidates.csv', 'utf-8'), { columns: true }) : [];
    const topMissing = new Map();
    enriched.forEach(e => {
        e.still_missing_fields.split(' | ').forEach(f => { if (f)
            topMissing.set(f, (topMissing.get(f) || 0) + 1); });
    });
...
${enriched.filter(e => e.promotion_candidate).slice(0, 5).map(e => `- **${e.product_name}** (${e.product_id}) - 신뢰도: ${e.evidence_confidence_summary}`).join('\n')}

## 4. 보강 실패 및 드랍 권고 (Failed)
${failed.slice(0, 3).map(e => `- **${e.product_name}**: ${e.failure_reason}`).join('\n')}
...
function getTopReasons(reports, field) {
    const map = {};
    reports.forEach(r => {
        const val = r[field];
        if (val && val !== 'NONE')
